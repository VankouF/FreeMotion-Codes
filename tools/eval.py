...
if __name__ == '__main__':
    ...
    parser.add_argument('--evalmodel_config', type=str, help='train config')
    ...
    data_cfg = get_config(args.dataset_config).interhuman_test
    cfg_path_list = [args.model_config]

    
    normalizer = MotionNormalizerTorch()

    spatial_mean = normalizer.motion_mean[:66]
    spatial_std = normalizer.motion_std[:66]
    ...
    for cfg_path in cfg_path_list:
        ...
        if model_cfg.CHECKPOINT and isinstance(model_cfg.CHECKPOINT, list):
        
            ckpts = []
            for ckpt_path in model_cfg.CHECKPOINT:
                ckpt = torch.load(ckpt_path, map_location="cpu")
                for k in list(ckpt["state_dict"].keys()):
                    if "model" in k:
                        ckpt["state_dict"][k.replace("model.", "")] = ckpt["state_dict"].pop(k)
                ckpts.append(ckpt['state_dict'])
            model.load_state_dict(ckpts, strict=True)
            print("checkpoint state loaded!")
        elif model_cfg.CHECKPOINT and isinstance(model_cfg.CHECKPOINT,str):
    ...
    evalmodel_cfg = get_config(args.evalmodel_config)
    eval_wrapper = EvaluatorModelWrapper(evalmodel_cfg, device)

    log_file = f'evaluation.log'
    evaluation(log_file)
