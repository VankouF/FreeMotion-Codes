...
from models.losses import InterLoss, GeometricLoss, SingleLoss, InitOrientationAndPositionLoss
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        # x.shape = (B,T,2D) = (1,210,2*262)
        # model_output.shape = (B,T,2D) = (1,210,2*262)
        ...
        return {
            "mean": model_mean,  # [1, 210, 524]
            "variance": model_variance,  # [1, 210, 524]
            "log_variance": model_log_variance,  # [1, 210, 524]
            "pred_xstart": pred_xstart,  # [1, 210, 524]
        }
    ...
    def ddim_sample_loop(
        ...
        return sample["pred_xstart"]  # [1, 210, 524]
...
#     def __init__(self, use_timesteps, motion_rep, archi, **kwargs):
...
#         x_and_xCondition = kwargs["x_start"]
#         B, T = x_and_xCondition.shape[:-1]

#         x_and_xCondition = x_and_xCondition.reshape(B, T, kwargs["model_kwargs"]['person_num'], -1)  # [B, T, 2, 262]
#         mask = mask.reshape(B, T, -1, 1)  # [B,T,2,1]

#         x_and_xCondition = self.normalizer.forward(x_and_xCondition)  # (target-mean)/std
        
#         ## new added
#         kwargs["x_start"] = x_and_xCondition[...,0,:].reshape(B, T, -1)
        
#         if kwargs["model_kwargs"]['person_num'] == 1:
#             kwargs["model_kwargs"]["motion_guidance"] = None
#         else:
#             kwargs["model_kwargs"]["motion_guidance"] = x_and_xCondition[...,1:,:]

#         items = super().training_losses(self._wrap_model(model), *args, **kwargs)
        
#         prediction  = items["pred"].reshape(B,T,1,-1)  # [B,T,262] -> [B,T,1,262]

#         target = items["target"].reshape(B,T,1,-1) # [B,T,262] -> [B,T,1,262]

#         timestep_mask = (kwargs["t"] <= t_bar).float()  # * cond_mask[0, 0]

#         if kwargs["model_kwargs"]['person_num'] == 1:
#             interloss_manager = SingleLoss("l2", 22)
#             prediction_pseudo = prediction
#             target_pseudo = target
#         else:
#             interloss_manager = InterLoss("l2", 22)
            
#             prediction_pseudo = torch.cat([prediction, kwargs["model_kwargs"]["motion_guidance"]],dim=-2)
#             target_pseudo = torch.cat([target, kwargs["model_kwargs"]["motion_guidance"]],dim=-2)
            
#         interloss_manager.forward(prediction_pseudo, target_pseudo, mask, timestep_mask)
        
#         loss_a_manager = GeometricLoss("l2", 22, "A")
#         loss_a_manager.forward(prediction[...,0,:], target[...,0,:], mask[...,0,:], timestep_mask)
        
#         # if self.archi =='multi_spatial':
#         #     loss_a_init_orientation = InitOrientationAndPositionLoss("l2", 22)
#         #     loss_a_init_orientation.forward(prediction[...,0,:], kwargs["model_kwargs"]["spatial_condition"])
...
#         if kwargs["model_kwargs"]['person_num'] > 1:
#             # if self.archi =='multi_spatial':
#             #     losses.update(loss_a_init_orientation.losses)
#             #     losses["total"] = loss_a_manager.losses["A"] + interloss_manager.losses["total"] + loss_a_init_orientation.losses["spatial"]
#             # else:
...
class MotionSpatialControlNetDiffusion(GaussianDiffusion):
    '''
    inject classifer spatial guidance
    '''
    def __init__(self, use_timesteps, motion_rep, archi, **kwargs):
        self.use_timesteps = set(use_timesteps)
        self.motion_rep = motion_rep
        ...
        self.spatial_mean = self.normalizer.motion_mean[:66]
        self.spatial_std = self.normalizer.motion_std[:66]
