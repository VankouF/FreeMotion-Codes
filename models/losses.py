...
class InitOrientationAndPositionLoss(nn.Module):
    def __init__(self, recons_loss, nb_joints):
        super(InitOrientationAndPositionLoss, self).__init__()
        self.nb_joints = nb_joints
        if recons_loss == 'l1':
            self.Loss = torch.nn.L1Loss(reduction='none')
        elif recons_loss == 'l2':
        ...
        self.weights = {}
        self.weights["init_ori"] = 0.01
        self.weights["init_pos"] = 3
        
        self.losses = {}

    def mse(self, prediction, target, weights=1.0):
        loss = self.Loss(prediction, target).mean(dim=-1, keepdim=True)
        loss = loss.sum() * weights
        return loss

    def forward(self, motion_pred, spatial_gt):
        
        B, T = motion_pred.shape[:2]
        
        prediction = self.normalizer.backward(motion_pred, global_rt=True)
        
        self.pred_g_joints = prediction[..., :self.nb_joints * 3].reshape(B, T, -1, self.nb_joints, 3)  # [B, T, 1, J, 3]
        
        r_hip, l_hip, sdr_r, sdr_l = face_joint_indx
        across = self.pred_g_joints[..., r_hip, :] - self.pred_g_joints[..., l_hip, :]
        across = across / across.norm(dim=-1, keepdim=True)
        
        y_axis = torch.zeros_like(across)  # [B, T, 1, 3]
        y_axis[..., 1] = 1
        
        pred_init_forward = torch.cross(y_axis, across, axis=-1)  # [B, T, 1, 3]????
        
        pred_init_forward = (pred_init_forward / pred_init_forward.norm(dim=-1, keepdim=True))[:,0,0,[0,2]]  # [B, T, 1, 3] -> [B, 2]
        
        pred_init_pos = self.pred_g_joints[..., 0, :][:,0,0,[0,2]]  # # [B, T, 1, J, 3] -> [B, T, 1, 3] -> [B, 2]
        
        self.losses['spatial'] = self.mse(pred_init_forward, spatial_gt[..., 0:2], self.weights['init_ori']) + self.mse(pred_init_pos, spatial_gt[..., 2:4], self.weights['init_pos'])
