...
def plot_3d_motion(save_path, kinematic_tree, mp_joints, title, figsize=(10, 10), fps=120, radius=4, hints=None):
    ...
    colors = ['red', 'green', 'black', 'blue', 'red',
              ...
              'darkred', 'darkred', 'darkred', 'black', 'blue']
    ...
    def update(index):
        ...
        # if hint is not None:
        #     ax.plot3D(hint[:index+1, 0], hint[:index+1, 1], hint[:index+1, 2], color="blue", linewidth=3.0)

        for idx, hint in enumerate(hints):
            if min(hint.shape) != 0:
                ax.plot3D(hint[:index+1, 0], hint[:index+1, 1], hint[:index+1, 2], color=colors[-1-idx], linewidth=3.0)
